...
- [X] T020 [P] [US1] Create TimescaleDB schema SQL (market_events hypertable with time column, health_alerts hypertable, composite index on asset+event_type+time per data-model.md) in docker/timescaledb/init.sql (mounted into TimescaleDB via Docker Compose; replaces the removed packages/nexus-ingestion/nexus_ingestion/persistence/schema.sql)
