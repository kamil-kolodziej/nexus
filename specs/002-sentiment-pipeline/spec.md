...
- What happens when the Redis connection to `nexus:sentiment-events` (output stream) is lost mid-publish during a fan-out of N scores?
  → The source message (`nexus:news-events`) is NOT acknowledged. The partially-published scores may be duplicated when the message is reprocessed after reconnection. Consumers must handle at-least-once delivery. The publisher sets `_connected=False` and buffers subsequent events in a bounded deque. **Note**: `REDIS_DISCONNECT` health alert emission and automatic reconnection (`flush_buffer()`/`reconnect()`) are declared in the design but not yet wired — see `docs/plans/TODO.md` (cross-package).
- What happens when the `nexus:news-events` stream is empty and no articles arrive?
  → The consumer blocks on `XREADGROUP` with a configurable timeout (default 5 seconds), then loops. No errors, no health alerts. The health endpoint reports `status: "ok"` with all component checks in `ok` state.
...
- **FR-010**: The service MUST expose a health endpoint at `GET /health` returning an RFC `application/health+json` body (per draft-inadarei-api-health-check) with a 3-state top-level `status` (`ok` / `degraded` / `error`), `serviceId`, `version`, and a `checks{}` object containing at minimum `processor:inference` and `redis:publisher` sub-checks. HTTP status MUST be `503` when top-level `status == "error"` and `200` otherwise. The top-level `status` MUST equal the worst of all component check statuses. Response time MUST be under 200 milliseconds. See `docs/design/nexus-trading-platform-design.md § Monitoring` for the platform-wide convention.
...
- **FR-019**: The extraction step MUST emit only assets, sectors, and companies that are present in the `active_assets` configuration list (loaded via `config.toml [sentiment] active_assets` or `NEXUS_ACTIVE_ASSETS` env var). Dictionary entries for unsupported instruments MAY exist for future use, but they MUST NOT produce published `SentimentScore` events until the corresponding asset, sector, or company canonical ID is added to `active_assets`.
...
- **Health response body**: Runtime status of the sentiment service exposed via `GET /health`, shaped per RFC draft-inadarei-api-health-check. Contains top-level `status` (`ok` / `degraded` / `error`, = worst of component checks), `serviceId` (`"nexus-sentiment"`), `version` (package version), and `checks{}` keyed by `componentName:measurementName` — at minimum `processor:inference` (with rolling-window failure count as `observedValue`) and `redis:publisher` (with buffer size as `observedValue`). Not a pydantic model — built as a plain dict at request time from in-memory counters.
...
- **SC-004**: The `GET /health` endpoint responds within 200 milliseconds, returns `application/health+json`, and accurately reflects component state in the `checks{}` object with top-level `status` equal to the worst component status. HTTP status is `503` when `status == "error"`, `200` otherwise.
